...
// Ajoute une légère rotation/ondulation aléatoire
const generateTransformAnimation = () => {
  const rotate = Math.random() * 60 - 30; // entre -30 et +30 degrés
  const wiggleX = Math.random() * 20 - 10; // oscille en X
  const wiggleY = Math.random() * 20 - 10; // oscille en Y

  return {
    transform: `rotate(${rotate}deg) translate(${wiggleX}px, ${wiggleY}px)`,
  };
};
...
const createAsteroid = () => {
  ...
  const transform = generateTransformAnimation();
  ...
  return (
    <div className={styles.asteroidContainer}>
      {asteroids.map((a) => (
        <div
          key={a.id}
          className={`${styles.asteroid} ${styles[a.direction]}`}
          style={{
            left: a.left,
            top: a.top,
            width: `${a.size}px`,
